...
/**
 * ZVIG Insurance Quote Flow — 6-step wizard (mock simulation).
 *
 * Step 1  Visitor details      -> writes `customers` when live
 * Step 2  Travel details       -> writes `travel_details`
...
import { calculatePremium, nextPolicyNumber, tripDays } from "@/lib/quote-engine";
...
export function QuoteWizard() {
  ...
  const pricing = useMemo(
    ...
    [product, form.arrivalDate, form.departureDate, form.dateOfBirth, form.activities]
  );
  ...
  const stepValid = (() => {
    switch (step) {
      ...
      case 1:
        return form.arrivalDate && form.departureDate && days > 0 && form.activities.length > 0;
      case 2:
            ...
            {step === 1 && (
              <Card>
                <CardContent className="p-7 sm:p-9">
                  ...
                  <p className="mt-1.5 text-sm text-stone-500">
                    Dates and activities set your cover period and price.
                  </p>
                  <div className="mt-7 grid gap-5 sm:grid-cols-2">
                    <div className="space-y-1.5">
                  ...
                  <dl className="mt-6 space-y-3 text-sm">
                    <div className="flex justify-between">
                      <dt className="text-stone-500">
                        Base premium · {pricing.days} days × {formatUSD(pricing.dayRate)}/day
                      </dt>
                      <dd className="font-medium text-stone-900">
                        {formatUSD(pricing.days * pricing.dayRate)}
                      </dd>
                    </div>
                    ...
                    {pricing.ageLoading > 1 && (
                      <div className="flex justify-between">
                        <dt className="text-stone-500">Age loading × {pricing.ageLoading.toFixed(1)}</dt>
                        <dd className="font-medium text-stone-900">included</dd>
                      </div>
                    )}
                    {pricing.minPremiumApplied && (
                      <div className="flex justify-between">
                        <dt className="text-stone-500">Minimum premium applied</dt>
                        <dd className="font-medium text-stone-900">{formatUSD(product.minPremiumUsd)}</dd>
                      </div>
                    )}
                    <div className="flex justify-between">
                      <dt className="text-stone-500">Subtotal</dt>
                      <dd className="font-medium text-stone-900">{formatUSD(pricing.subtotal)}</dd>
                    </div>
                    <div className="flex justify-between">
                      <dt className="text-stone-500">Regulatory levy (5%)</dt>
                      <dd className="font-medium text-stone-900">{formatUSD(pricing.levies)}</dd>
                    </div>
              ...
              <Card>
                <CardContent className="p-7 sm:p-9">
                  ...
                  <div className="mt-7 flex items-center justify-between rounded-xl bg-stone-50 px-5 py-4">
                    <span className="text-sm text-stone-500">Total due today</span>
